# Trifan — Deployment (standalone)

Standalone product. **Not** merged with FANN, **not** dependent on any FANN backend/repo.

- Frontend (`frontend/`) → **Vercel** → `trifan.com`
- Backend (`backend/`) → **Render** (recommended) → `api.trifan.com`
...
3. Env var: `VITE_API_BASE_URL = https://api.trifan.com/api`
4. Add domain `trifan.com` (+ `www`).
...
5. Env vars (blueprint sets most): `DJANGO_SETTINGS_MODULE=core.settings_production`, `DJANGO_DEBUG=false`, `DJANGO_SECRET_KEY` (generate), `DJANGO_ALLOWED_HOSTS=api.trifan.com`, `CORS_ALLOWED_ORIGINS=https://trifan.com,https://www.trifan.com`, `CSRF_TRUSTED_ORIGINS=https://trifan.com,https://www.trifan.com,https://api.trifan.com`, `FRONTEND_BASE_URL=https://trifan.com`.
6. Add domain `api.trifan.com`.
...
| Type | Name | Value |
|---|---|---|
| A / CNAME | `trifan.com` (+ `www`) | Vercel target (shown in Vercel domain settings) |
...
- Frontend prod build: `vite build` ✓; `api.trifan.com` baked in; **no** `localhost`/`api.fann.art` leaks.
...
1. **GitHub**: authorize me / confirm to create repo `trifan` and push (repo is staged locally at `~/Downloads/trifan`).
2. **Vercel**: log in / authorize to import the repo + add `trifan.com`.
3. **Render** (or chosen host): log in / authorize to deploy `backend/` + add `api.trifan.com`.
4. **DNS**: access to the `trifan.com` registrar to add the A/CNAME records above.
