...
## Structure (monorepo)
- `frontend/` — Vite + React + TypeScript SPA → deploys to **Vercel** (`trifan.com`).
- `backend/`  — Django + DRF API (SQLite now, Postgres in prod) → deploys to a
  Python host, e.g. **Render** (`api.trifan.com`).
...
- Frontend: Vercel, root dir `frontend/`, env `VITE_API_BASE_URL=https://api.trifan.com/api`.
